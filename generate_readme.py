...
def add_image(readme, img_name):
    img_url = "https://github.com/autonlab/pmx_data/blob/main/images/" + img_name + ".png?raw=true"
    readme.add_element(snakemd.Paragraph([snakemd.InlineText(img_name, url=img_url, image=True)]))

def generate_readme(location, base_url):
    ...
    add_image(readme, "equipment-type")
    add_image(readme, "pmx-tasks")
    add_image(readme, "ml-algorithms")
    ...
    for dirname in sorted(os.listdir("pmx_data")):
        ...
        name = info["name"]
        namelink = "[" + name + "](" + base_url + dirname + ")"
    ...
    with open(readme_path, 'w') as readme_file:
        readme_file.write(readme.render())

#github actions will input branch name as command line arg
branchname = sys.argv[1]
generate_readme(location=".", base_url="https://github.com/autonlab/pmx_data/tree/" + branchname + "/pmx_data/")
generate_readme(location="docs", base_url="http://autonlab.org/pmx_data/data/")